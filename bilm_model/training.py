...
class LanguageModel(object):
    ...
    def _build_loss(self, lstm_outputs):
        ...
        with tf.variable_scope('softmax'), tf.device('/cpu:0'):
            ...
            self.softmax_b = tf.get_variable( 'b', [n_tokens_vocab], dtype=DTYPE,
                initializer=tf.constant_initializer(0.0))
        ...
        for id_placeholder, lstm_output_flat in zip(next_ids, lstm_outputs):
            ...
            with tf.control_dependencies([lstm_output_flat]):
                if self.is_training and self.sample_softmax:
                    losses = tf.nn.sampled_softmax_loss(self.softmax_W, self.softmax_b, next_token_id_flat,
                                                        lstm_output_flat, self.options['n_negative_samples_batch'],
                                                        self.options['n_tokens_vocab'], num_true=1)
                    ...
                    #   softmax internally
                    losses = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=output_scores,
...
def train(options, data, n_gpus, tf_save_dir, tf_log_dir,
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        if restart_ckpt_file is not None:
            ...
            saver.restore(sess, tf.train.latest_checkpoint(restart_ckpt_file))
        ...
        unroll_steps = options['unroll_steps']  # 10
        n_train_tokens = options.get('n_train_tokens')  # 99999
        n_tokens_per_batch = batch_size * unroll_steps * n_gpus  # 128*10*1
        n_batches_per_epoch = int(n_train_tokens / n_tokens_per_batch)  # 99999/1280
        n_batches_total = options['n_epochs'] * n_batches_per_epoch  # 3*99999/1280
        ...
        if not char_inputs:
            feed_dict = {model.token_ids: np.zeros([batch_size, unroll_steps], dtype=np.int64)
                for model in models}
        else:
            feed_dict = {model.tokens_characters: np.zeros([batch_size, unroll_steps, max_chars], dtype=np.int32)
                for model in models}

        if bidirectional:
            if not char_inputs:
                feed_dict.update({model.token_ids_reverse: np.zeros([batch_size, unroll_steps], dtype=np.int64)
                    for model in models})
            else:
                feed_dict.update({model.tokens_characters_reverse: np.zeros([batch_size, unroll_steps, max_chars], dtype=np.int32)
                    for model in models})
        ...
        for batch_no, batch in enumerate(data_gen, start=1):
            ...
            if batch_no % 1000 == 0:
                ...
                with h5py.File(str(batch_no)+'_vocab_embedding.hdf5', 'w') as fout:
                    fout.create_dataset('embedding', shape=embed.shape, dtype='float32', data=embed)
